...
def _transition_subtask_status_error_message(
	before_status: str, after_status: str, required_before: str
) -> str:
	before_txt = helper._escape_html(before_status)
	after_txt = helper._escape_html(after_status)
	req_txt = helper._escape_html(required_before)
	default = (
	...
	return helper._render_response_text(
		"transition_error",
		default,
		{
...
def _validate_transition(before_status: str, after_status: str) -> Optional[str]:
	...
	return _transition_subtask_status_error_message(
		before_norm, after_norm, required_before
	)
...
def _is_sender_task_pic(sender_username: str, task_id: str) -> bool:
	...
	employee_ids = frappe.get_all(
		"Task PIC", filters={"parent": task_id}, pluck="employee"
	)
...
def _expected_subtask_scope_settings(message) -> dict:
	return helper._conf_default_subtask_settings(message)


def _expected_subtask_scope_private_settings(message, subtask_id) -> dict:
	username = (message.from_user.username or "").strip().lstrip("@").lower()
	if not username:
		return {}

	requestor = frappe.db.get_value(
		"SubTask", {"requestor": "@" + username}, "requestor"
	)
...
def _scope_subtask_update_error_message(settings: dict) -> str:
	...
	if not maintask_name and expected_maintask:
		try:
			maintask_name = (
				frappe.db.get_value("MainTask", expected_maintask, "maintask_name")
				or ""
			).strip()
		except Exception:
			maintask_name = ""
	...
	if expected_tasks:
		try:
			tasks_name = (
				frappe.db.get_value("Tasks", expected_tasks, "task_name") or ""
			).strip()
		except Exception:
			tasks_name = ""

	maintask_label = (
		maintask_name or expected_maintask or "maintask tidak terkonfigurasi"
	)
	tasks_label = tasks_name or expected_tasks or "task tidak terkonfigurasi"
	default = "❌ Subtask bukan bagian dari maintask <b>{maintask_label}</b> dan task <b>{tasks_label}</b> yang ditentukan"
	return helper._render_response_text(
		"scope_error",
		default,
		{
			"maintask_label": helper._escape_html(maintask_label),
			"tasks_label": helper._escape_html(tasks_label),
		},
	)
...
def _sender_error_message() -> str:
	return helper._render_response_text(
		"sender_username_missing", "❌ Username Telegram tidak ditemukan"
	)
...
def _subtask_link(subtask_id: str) -> Tuple[str, str]:
	...
	link = helper._format_hyperlink(subtask_id, url) if base else subtask_id
	return link, url


def aduan_update_status_response(
	payload: str, message, command_token: str, is_private: Optional[bool] = None
) -> str:
	...
	if is_private and not is_requestor:
		return helper._render_response_text(
			"not_requestor_permission_denied",
			"❌ Anda tidak dapat update subtask ini karena Anda bukan requestor dari subtask {subtask_id} - {subtask_title}",
			{
				"subtask_id": doc.name,
				"subtask_title": doc.subtask_name,
			},
		)
	...
	if not pic_name:
		...
		if pic:
			try:
				pic_name = (
					frappe.db.get_value("Employee", pic, "employee_name") or ""
				).strip()
			except Exception:
				pic_name = ""
	...
	if new_status == "Resolved":
		...
		return helper._render_response_text(
			...
			default,
			{
				...
				"pic_name": helper._escape_html(pic_name),
			},
		).strip()

	# Done
	default = (
		"🟢 Issue {link} {requestor} sudah selesai ditangani (Done) oleh {pic_name}"
	)
	return helper._render_response_text(
...
def aduan_update_issues_response(payload: str, message, command_token: str) -> str:
	...
	if not maintask_name and maintask:
		try:
			maintask_name = (
				frappe.db.get_value("MainTask", maintask, "maintask_name") or ""
			).strip()
		except Exception:
			maintask_name = ""
	...
	try:
		issue_docname, issue_label = helper._resolve_issue_type(
			issues_input, maintask, maintask_name
		)
	...
	if issues_input:
		pics = helper._pics_for_issue_user_input(issues_input)
		if pics:
			pic_issue = ", ".join(pics)
	...
	return helper._render_response_text(
		...
		{
			"link": link,
			"issue_label": helper._escape_html(issue_label),
