        ...
        summ_data.append(
            {
                ...
                "open_maintask": frappe.db.count(
                    "MainTask", filters={"name": tsm.mt_name, "status": "Open"}
                ),
                "hold_maintask": frappe.db.count(
                    "MainTask", filters={"name": tsm.mt_name, "status": "Hold"}
                ),
                "cancel_maintask": frappe.db.count(
                    "MainTask", filters={"name": tsm.mt_name, "status": "Cancel"}
                )
            }
        )
        # tsm["total_maintask"] = frappe.db.count("MainTask", filters={"name": tsm.mt_name})
        # tsm["completed_maintask"] = frappe.db.count(
        #     "MainTask", filters={"name": tsm.mt_name, "status": "Done"}
        # )
        # tsm["open_maintask"] = frappe.db.count(
        #     "MainTask", filters={"name": tsm.mt_name, "status": "Open"}
        # )
        # tsm["hold_maintask"] = frappe.db.count(
        #     "MainTask", filters={"name": tsm.mt_name, "status": "Hold"}
        # )
        # tsm["cancel_maintask"] = frappe.db.count(
        #     "MainTask", filters={"name": tsm.mt_name, "status": "Cancel"}
        # )
...
def get_chart_data(data):
    ...
    for tsm in data:
        ...
        open.append(tsm["open_maintask"])
        completed.append(tsm["completed_maintask"])
        hold.append(tsm["hold_maintask"])
        cancel.append(tsm["cancel_maintask"])
...
def get_report_summary(data):
    ...
    avg_completion = sum(tsm["completed_maintask"] for tsm in data) / len(data)
    total = sum([tsm["total_maintask"] for tsm in data])
    # total = len(set(tsm.mt_name for tsm in data))
    completed = sum([tsm["completed_maintask"] for tsm in data])
