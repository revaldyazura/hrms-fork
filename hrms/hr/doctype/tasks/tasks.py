	...
	if previous_status != now_status:
		...
		for name in subtasks:
			...
			if subtask.status not in ("Done", "Close", "In Progress", "Pause"):
				...
				frappe.msgprint(f"SubTask {subtask.subtask_name} status is {subtask.status}, not updated.")
...
def has_permission(doc, ptype, user):
	...
	if ptype == "delete":
		...
		print(f'ptype delete, parent_task_pic value {parent_task_pic}')
		if is_task_pic and  is_maintask_owner and  in_assign_by_list:
			frappe.throw(
		...
		if is_owner or is_task_pic or is_maintask_owner:
			...
			frappe.throw(f"{employee.employee_name} is not allowed to edit {doc.task_name} task", frappe.PermissionError)
			return False
	elif ptype == "create":
		if in_assign_by_list:
			return True
		else:
			frappe.throw(f"{employee.employee_name} is not allowed to create task in {maintask.maintask_name} maintask.", frappe.PermissionError)
			return False
		

	frappe.throw(f"{employee.employee_name} is not allowed to accessing {doc.task_name} task.", frappe.PermissionError)
	return False
...
def get_template_details(template_name):
	return frappe.get_all("SubTask Template Detail", filters={"parent": template_name}, fields=["subtask_name_template","value_template", "description_template", "target_time_template", "unit_target_time_template", "status_template", "type_template", "priority_template"])
