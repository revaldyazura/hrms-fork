...
class Tasks(Document):
	...
	def validate(self):
		print(f"validate task {self.task_name} owner {self.owner}, id {self.name}")
...
def _lock_maintask_for_maintask_update(maintask_id: str):
	...
	frappe.db.sql(
		"SELECT name FROM `tab{}` WHERE name=%s FOR UPDATE".format("MainTask"),
		maintask_id,
	)


def _append_child_table_maintask_row_if_missing(
	parent_doc, employee_id: str, employee_name: str | None, child
):
	if not employee_id:
		return False
	...
	# set employee_name from parameter if provided, otherwise fetch from DB
	if employee_name:
...
def ensure_employee_in_maintask_child_table(doc: Document | str):
	# accept either doc object or name
	if isinstance(doc, str):
		doc = frappe.get_doc("Tasks", doc)

	rows_task_pic = getattr(doc, "task_pic", []) or []

	parent_id = getattr(doc, "maintask", None)
	if not parent_id:
		# nothing to sync if task is not linked to a maintask
		return

	# lock once per maintask update
	_lock_maintask_for_maintask_update(parent_id)
	parent = frappe.get_doc("MainTask", parent_id)

	any_added = False
	for row in rows_task_pic:
		# row may be a child doc object or simple mapping; extract employee id
		emp_id = getattr(row, "employee", None) if hasattr(row, "employee") else None
		if not emp_id and isinstance(row, dict):
		...
		emp_reports_to_name = (
			frappe.db.get_value("Employee", emp_reports_to, "employee_name")
			if emp_reports_to
			else None
		)

		team_added = _append_child_table_maintask_row_if_missing(
			parent, emp_id, emp_name, "team"
		)
		assign_by_added = (
			_append_child_table_maintask_row_if_missing(
				parent, emp_reports_to, emp_reports_to_name, "assign_by"
			)
			if emp_reports_to
			else False
		)

		if team_added or assign_by_added:
			any_added = True
			messages = []
			if team_added:
				messages.append(
					f"Employee <b>{frappe.utils.escape_html(emp_name or emp_id)}</b> added to Team MainTask <b>{frappe.utils.escape_html(parent.maintask_name)}</b>."
				)
			if assign_by_added:
				messages.append(
					f"Employee <b>{frappe.utils.escape_html(emp_reports_to_name or emp_reports_to)}</b> added to Assign By MainTask <b>{frappe.utils.escape_html(parent.maintask_name)}</b>."
				)
